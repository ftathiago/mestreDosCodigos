...
type
  ...
  private
    ...
    procedure ExecutarDelete(const NomeTabela: string);
  public
...
procedure TmcMetaDataController.CarregarNosDataSet(Entidade, EntPropriedade: TDataSet;
  const LimparDataSet: boolean);
begin
...
begin
  ...
  with FPropriedade.Indexes.Add do
  begin
    Active := True;
    Selected := True;
    Name := 'IDX_COLUMN_NAME';
    Fields := 'COLUMN_NAME';
  end;
...
procedure TmcMetaDataController.ExecutarDelete(

  const
    NomeTabela:
    string);
begin
  FConnection.ExecSQL(Format('delete from %s', [NomeTabela]));
end;

procedure TmcMetaDataController.FiltrarPropriedades(const ANomeEntidade: string);
begin
  if ANomeEntidade.Trim.IsEmpty then
    FPropriedade.Filter := EmptyStr;

  FPropriedade.Filter := Format('TABLE_NAME = %s', [ANomeEntidade.ToUpper.QuotedString]);
  FPropriedade.Filtered := not ANomeEntidade.Trim.IsEmpty;
end;
...
procedure TmcMetaDataController.Limpar;
begin
  FConnection.StartTransaction;
  try
    ExecutarDelete(Entidade);
    ExecutarDelete(ENT_PROPRIEDADE);
    FConnection.Commit;
  except
...
class
  function TmcMetaDataController.New(AConnection: TFDConnection): ImcMetaDataController;
begin
